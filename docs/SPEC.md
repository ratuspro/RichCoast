# Rich Coast — 2D Arcade, Mobile

> Gameplay design. Engine-agnostic and carried over unchanged from the original Phaser build;
> the Unity architecture that implements it is in [TECH_SPEC.md](TECH_SPEC.md). Where this
> document names a `src/**.ts` file, see the mapping table in TECH_SPEC.md for its Unity
> counterpart.
...
- Native mobile: **Android first**, iOS to follow.
- Runs at 60 fps on a ~2019 budget Android phone (Snapdragon 4xx class, 2–3 GB RAM).
- Locked portrait; the 390×844 figures below are the design space, fitted to the device
  aspect ratio and safe area at runtime.
- Engine: Unity 6000.5 LTS, URP, 2D physics (Box2D) with rendering free to become 3D.
- Visuals are placeholder during the migration and get a dedicated art pass afterwards;
  the "Bright Workshop" theme below is the design intent, not the current build.

---

## Architecture

Each zone is a self-contained system coupled to the others only through a single event
contract. See [TECH_SPEC.md](TECH_SPEC.md) for the assembly layout, the interface contract,
and how a zone is built and played in isolation while the others are stubbed.
