 ...
 *
 * The world is taller than the 390×844 screen (Zone B's band ends at world y=952) and the
 ...
 *    full 326px board band. Zone B pokes in at the bottom, cropped by PAN_DISTANCE.
 *  - B-phase (pan 108): main camera scrollY 108; the arena viewport shrinks to 218px —
 *    the arena camera keeps its floor-anchored centring (ArenaView), so Zone A is TOP-cropped
 *    at unchanged zoom. Zone B's full 540px band exactly fills the screen down to y=844.
 *
...
export const ARENA_VIEW_H_A = Layout.zoneA.height - HUD_H; // 326

/** Arena camera viewport height in the B-phase: the old 272px band × 0.8 (top-cropped). */
export const ARENA_VIEW_H_B = 218;

/**
 * How far the main camera scrolls down between the phases. Three ways to derive it, all
 * equal by construction (asserted in tests): the arena viewport shrink (326−218), Zone B's
 * world overhang below the screen (952−844), and Zone C's on-screen shift (368−260).
 */
export const PAN_DISTANCE = ARENA_VIEW_H_A - ARENA_VIEW_H_B; // 108
